...
class _AddResortForm extends StatefulWidget {
  const _AddResortForm({Key? key}) : super(key: key);

  @override
  State<_AddResortForm> createState() => _AddResortFormState();
}

class _AddResortFormState extends State<_AddResortForm> {
            ...
            child: Column(
              ...
              children: [
                ...
                BlocConsumer<ResortsCubit, ResortsState>(
                  listener: (context, state) {
                    if (state.add is ResortAddSuccess ||
                        state.add is ResortAddFailed) {
                      imgProv.reset();
                      Navigator.pop(context);
                    }
                  },
                  builder: (context, state) {
                    if (state.add is ResortAddLoading) {
                ...
                AppButton(
                  ...
                  onPressed: () {
                    if (forms.addResortState.currentState!.validate()) {
                      final fields = forms.addResortState.currentState!.fields;
                      ...
                      Resort resort = Resort(
                        ...
                        isPopular: fields['isPopular']!.value ?? false,inclusiveCharges: double.parse(
                            fields['charges']!.value.toString().trim()),
                      );

                      ResortsCubit.c(context).add(resort, imgProv.images);
                    }
                  },
                ),
              ],
