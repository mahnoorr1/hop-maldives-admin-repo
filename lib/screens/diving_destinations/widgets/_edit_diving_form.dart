...
class _EditDivingForm extends StatefulWidget {
  final Diving diving;
  final int index;
  const _EditDivingForm({
    Key? key,
  ...
  @override
  State<_EditDivingForm> createState() => _EditDivingFormState();
}

class _EditDivingFormState extends State<_EditDivingForm> {
  ...
  @override
  void initState() {
    types = widget.diving.types;
    imagesURLs = widget.diving.images;
    activities = widget.diving.activities.toMap();
    super.initState();
  }
    ...
    return Material(
      ...
      child: Container(
        ...
        child: SingleChildScrollView(
          padding: Space.all(1),
          child: FormBuilder(
            key: forms.editDivingState,
            initialValue: {
              'name': widget.diving.name,
              'address': widget.diving.address,
              'description': widget.diving.description,
              'isPopular': widget.diving.isPopular,
              'lat': widget.diving.lat.toString(),
              'lng': widget.diving.lng.toString(),
            },
            child: Column(
              ...
              children: [
                ...
                Space.y!,
                if (widget.diving.types.isNotEmpty) ...[
                  Wrap(
                ...
                Row(
                  children: [
                    ...
                    IconButton(
                      onPressed: () {
                        String? value = forms.editDivingState.currentState!
                            .fields['type']!.value;
                        if (value != null && value.toString().isNotEmpty) {
                          setState(() {
                            types.add(value.toString().trim());
                          });
                          forms.editDivingState.currentState!.fields['type']!
                              .reset();
                        }
                      },
                      icon: const Icon(
                ...
                BlocConsumer<DivingCubit, DivingState>(
                  listener: (context, state) {
                    if (state.update is DivingUpdateSuccess ||
                        state.update is DivingUpdateSuccess) {
                      imgProv.reset();
                      Navigator.pop(context);
                    }
                  },
                  builder: (context, state) {
                    if (state.update is DivingUpdateLoading) {
                ...
                AppButton(
                  ...
                  onPressed: () {
                    if (forms.editDivingState.currentState!.validate()) {
                      final fields = forms.editDivingState.currentState!.fields;
                      ...
                      Diving diving = Diving(
                        ...
                        types: types,
                        reviews: widget.diving.reviews,
                        images: widget.diving.images,
